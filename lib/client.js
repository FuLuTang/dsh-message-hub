window.__ModuleLoader__.load({ id: 'dsh-message-hub', factory: (require) => {
  ...
  function HubPanel() {
    ...
    return React.createElement('div', { style: { position: 'relative' } },
      React.createElement('button', { type: 'button', title: 'Message Hub 渠道', onClick: () => setOpen(!open) }, `消息渠道 · ${(snapshot.channels || []).filter((c) => c.light === 'green').length}/${(snapshot.channels || []).length}`),
      open && React.createElement('div', { style: { position: 'absolute', right: 0, top: '100%', zIndex: 50, width: 360, maxHeight: 480, overflow: 'auto', padding: 12, background: 'var(--dsh-color-bg, #18181b)', color: 'var(--dsh-color-fg, #fafafa)', border: '1px solid #52525b', borderRadius: 8 } },
        React.createElement('strong', null, 'Message Hub 渠道'), React.createElement('button', { style: { float: 'right' }, onClick: refresh }, '刷新'),
  ...
  function apply(ctx) { ctx.slots.inject('conversation.session.header.utilities', () => ctx.slots.register({ name: 'conversation.session.header.utilities', id: 'message-hub-trigger', order: -40 }, HubPanel)) }
